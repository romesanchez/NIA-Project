...
const DEVICE_ICONS: Record<string, string> = {
  ...
  SMARTPHONE: "📱", MEDIACONVERTER: "🔌", CONTROLLER: "🎛️", SATELLITE: "📡",
};
...
export function DepartmentSlide({ dept, tinted, onOpen2D }: Props) {
  const navigate = useNavigate();
  const { isAdmin } = useAppMode();

  // Live device counts — always in sync with the topology canvas. Updates
  // automatically when devices are added or removed in the editor.
  const liveDevices = useLiveDevices(dept.acronym, dept.devices);
  const deviceEntries = Object.entries(liveDevices).filter(([, v]) => v && v > 0);
