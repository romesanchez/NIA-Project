 ...
 * `text` supports multiple lines (\n-separated) so it can hold small notes
 * like a VLAN table or device credentials, not just a single word. */
export interface LLabel { id: string; x: number; y: number; text: string; boxed?: boolean }

/** Sizing constants + helper for rendering a multi-line label's background
 * box. Shared between the topology editor (while typing) and the read-only
 * viewer, so a note always measures the same everywhere it's drawn. */
export const LABEL_FONT_SIZE = 11;
export const LABEL_LINE_HEIGHT = 15;
export const LABEL_PAD_X = 16;
export const LABEL_PAD_Y = 12;
export const LABEL_MIN_W = 110;
// Fallback estimate, only used when canvas measurement isn't available
// (e.g. during SSR). Bumped up from 6.4 and paired with a safety margin
// below — the old value under-measured bold text in the actual label font,
// which is what let long lines spill past the drawn box edge.
export const LABEL_CHAR_W = 7.4;
// Extra breathing room added on top of whatever width we measure, so
// rounding/kerning differences between the measuring font and the on-screen
// font never eat into the last character or two.
const LABEL_WIDTH_SAFETY = 6;

// A single offscreen canvas, reused across calls, to measure text at the
// exact font/weight/letter-spacing the label is actually rendered with
// (see the textarea + <text> styles in TopologyEditor/TopologyViewer).
// This replaces the old fixed-char-width guess, which is what caused long
// lines to overflow the box — a guessed width can't account for real
// glyph widths, and bold monospace-ish fonts are rarely truly monospaced.
let _labelMeasureCtx: CanvasRenderingContext2D | null | undefined;
function getLabelMeasureCtx(): CanvasRenderingContext2D | null {
  if (_labelMeasureCtx !== undefined) return _labelMeasureCtx;
  if (typeof document === "undefined") { _labelMeasureCtx = null; return null; }
  const canvas = document.createElement("canvas");
  _labelMeasureCtx = canvas.getContext("2d");
  if (_labelMeasureCtx) {
    // Match the label's actual CSS font: 700 weight, LABEL_FONT_SIZE px,
    // the app's mono stack. Falls back to a generic monospace if the CSS
    // variable can't be resolved outside a styled document.
    const fontVar = typeof window !== "undefined"
      ? getComputedStyle(document.documentElement).getPropertyValue("--font-mono").trim()
      : "";
    _labelMeasureCtx.font = `700 ${LABEL_FONT_SIZE}px ${fontVar || "monospace"}`;
  }
  return _labelMeasureCtx;
}

/** Width of one line of label text, in px, including the same letter-spacing
 * (0.03em) applied in the rendered <text>/<textarea>. Falls back to the
 * fixed-width estimate if canvas measurement isn't available. */
function measureLabelLine(line: string): number {
  const ctx = getLabelMeasureCtx();
  if (!ctx) return line.length * LABEL_CHAR_W;
  const letterSpacing = LABEL_FONT_SIZE * 0.03;
  const base = ctx.measureText(line).width;
  // letter-spacing adds space after every character except the last
  const extra = Math.max(0, line.length - 1) * letterSpacing;
  return base + extra;
}

export function computeLabelBox(text: string): { lines: string[]; width: number; height: number } {
  const lines = (text || "").split("\n");
  const widestLine = Math.max(1, ...lines.map((l) => measureLabelLine(l)));
  const width = Math.max(LABEL_MIN_W, widestLine + LABEL_PAD_X + LABEL_WIDTH_SAFETY);
  const height = Math.max(LABEL_LINE_HEIGHT + LABEL_PAD_Y, lines.length * LABEL_LINE_HEIGHT + LABEL_PAD_Y);
